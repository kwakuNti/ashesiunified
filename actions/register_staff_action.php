...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirm_password) {
        header("Location: ../templates/Registerstaff.php?msg=Passwords do not match. Please try again.");
        exit;
    }
        ...
        if ($result->num_rows > 0) {
            header("Location: ../templates/Registerstaff.php?msg=User already registered");
            exit;
        }
        ...
        if ($results->num_rows > 0) {
                header("Location: ../templates/Registerstaff.php?msg=User Id already registered");
                exit;
        }
    ...
    $stmt->bind_param("issssssis", $staff_id, $first_name, $last_name, $phone_number, $dob, $email, $hashed_password, $roleID, $statusID);
    $stmt->execute();
    ...
    $stmt_pic->bind_param("is",$staff_id,$image_path);
    $stmt_pic->execute();
    // Check if the insertion was successful
    if ($stmt->affected_rows > 0 && $stmt_student->affected_rows > 0 && $stmt_pic->affected_rows > 0) {
        // Registration successful
        header("Location: ../templates/Login.php?msg=Registered Successfully");
        exit;
    } else {
        // Registration failed
        header("Location: ../templates/Registerstaff.php?msg=Failed to register staff. Please try again");
    }
