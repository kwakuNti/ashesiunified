...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirm_password) {
        ...
        if ($result->num_rows > 0) {
            header("Location: ../../pages/Register/Registerstaff.php.php?msg=User already registered");
            exit;
        }
    ...
    $sql_student = "INSERT INTO Staff (staff_id,unique_id,department_id, position_id)
                    VALUES (?, ?,?, ?)";
    $stmt_student = $conn->prepare($sql_student);
    $stmt_student->bind_param("iiii", $staff_id,$unique_id,$department_id,$position_id);
    ...
    if ($stmt->affected_rows > 0 && $stmt_student->affected_rows > 0 && $stmt_pic->affected_rows > 0) {
        ...
        // Registration failed
        header("Location: ../Resgister/Registerstaff.php.php?msg=Failed to register staff. Please try again");
    }
